...
- `node` module and P2P messages from the `message` module (except for
  `AnyTx` and `Precommit`) were moved to a separate crate, `vega-node`.
...
- Rust runtime module was moved from the `vega` crate into the separate
  `vega-rust-runtime` crate. (#1641)
...
- `api` module of the `vega` crate has been moved to 2 separate crates (#1690):

  - `vega-api` crate defining the base HTTP wrapper
  - `vega-system-api` crate implementing system API endpoints
...
- `ExecutionContext` has been reworked (#1711)
  ...
  - `ExecutionContext` has been extended by the methods from the
   `vega_rust_runtime::CallContext` so there is no need to use `CallContext`
    anymore.
...
- Node logic (including P2P networking and consensus algorithm) was moved
  from the `vega` crate into the separate `vega-node` crate. (#1698)
...
- Introduced a new crate `vega-proto`. Trait `ProtobufConvert` is moved
  to this crate. (#1496)

#### vega-protobuf-convert

- Introduced a new crate `vega-protobuf-convert`. Derive macro
...
- Derive macro `ProtobufConvert` now does not derive the `BinaryValue` and
  ...
  the `vega-derive` crate. (#1501)

#### vega-build

- Method `protobuf_generate` is now private, use `vega_build::ProtobufGenerator`
  instead (#1496).
...
- Methods `read_keys_from_file` and `generate_keys` are moved to new `keys`
  module in the `vega`. (#1459)

- Protobuf serialization for crypto types is now implemented in the `vega-crypto`
  crate. (#1496)
...
In `vega` 0.13, a new service workflow is introduced, named
"Dynamic Services".

Key points of this feature are the following:

- `vega` now supports different environments of code execution (runtimes).
  ...
  [runtime-trait]: https://docs.rs/vega/0.1.0/vega/runtime/trait.Runtime.html
  [sample-runtime]: https://github.com/vega/vega/tree/v0.1.0/examples/sample_runtime
...
- Services are not statically tied to the compiled binary anymore. There is
  ...
  [runtime-docs]: https://docs.rs/vega/0.1.0/vega/runtime/index.html
...
- `configuration` service was replaced with the `supervisor` service, which is
  ...
  [supervisor]: https://github.com/vega/vega/tree/v0.1.0/services/supervisor
...
[crypt-0-12]: https://github.com/vega/vega/blob/v0.12.1/examples/cryptocurrency/
[crypt-0-13]: https://github.com/vega/vega/blob/v0.1.0/examples/cryptocurrency/
...
- Services do not launch at the node start by default. For launching a
  service, use an [`vega-launcher`][launcher] tool.

  [launcher]: https://github.com/vega/vega-launcher
...
- #1537: Finalize Vega-derive macros [ECR-3800]
...
- #1432: Refine vega-derive crate on top of darling [ECR-3343]

- #1434: Replace `dispatcher` attribute in `vega_service`
  by the `service_interface` in `ServiceFactory` [ECR-3474]

- #1438: Remove dispatcher reference from Runtime trait

- #1443: Replace fabric module with vega-cli crate [ECR-3457]
...
- #1488: Add support of external runtimes to vega-cli
...
- #1539: Restore warn(missing_docs) in the Vega crate [ECR-3821]

- #1540: Deploy workflow

- #1542: Write proper examples for the Vega traits derivation [ECR-3822]
...
- `vega` now has a `python` library for implementing integration tests. (#1516)
...
- Old `fabric` module is replaced with new `vega-cli` crate. (#1443)

- `vega-cli` provides a public reexport of `structopt` crate. (#1461)
...
- `vega-protobuf-convert` has been replaced with external `protobuf-convert`
  crate. (#1561)

- `keys` module has been moved into `vega-keys` crate. (#1497)
...
- Configuration change example has been moved to `vega-supervisor` crate. (#1582)
...
- Module `storage` has been replace by `vega-merkledb` crate. See related section
...
- Module `storage` has been replaced by `vega-merkledb` crate. See related section
...
- Bootstrapping workflow has been simplified (#1292)
  ...
  ```bash
  cargo run --bin vega-timestamping -- \
    generate-template /tmp/vega/template.toml --validators-count 4

  cargo run --bin vega-timestamping -- \
    generate-config /tmp/vega/template.toml /tmp/vega/cfg/0 \
      --peer-address 0.0.0.0:8000
  cargo run --bin vega-timestamping -- \
    generate-config /tmp/vega/template.toml /tmp/vega/cfg/1 \
      --peer-address 0.0.0.0:8001
  cargo run --bin vega-timestamping -- \
    generate-config /tmp/vega/template.toml /tmp/vega/cfg/2 \
      --peer-address 0.0.0.0:8002
  cargo run --bin vega-timestamping -- \
    generate-config /tmp/vega/template.toml /tmp/vega/cfg/3 \
      --peer-address 0.0.0.0:8003

  cargo run --bin vega-timestamping -- \
    finalize /tmp/vega/nodes/0/sec.toml /tmp/vega/nodes/0/node.toml \
      --public-configs /tmp/vega/cfg/{0,1,2,3}/pub.toml

  cargo run --bin vega-timestamping -- \
    run -d /tmp/vega/db/0 -c /tmp/vega/nodes/0/node.toml
  ```
...
- Removed deprecated `CryptoHash` trait, use `vega-merkledb::ObjectHash`
  instead (#1361)
...
- `rocksdb` crate is now used instead of `vega_rocksdb`. (#1286)
...
- All Vega crates have been updated to Rust 2018 edition. This means that
...
- `transactions!` macro has been removed, `TransactionSet` derive macro
  from `vega-derive` should be used instead. (#1088)
...
- Added `MapProof::all_entries_unchecked` method. It is used for more efficient
  calculations in Vega Java Bindings, but can be used for debug purposes
  as well. (#918)
...
- `vega-build` crate has been added to simplify writing `build.rs` files
  for services that use protobuf code generation. (#1076)

#### vega-derive

- `vega-derive` crate has been added with custom derives for `ProtobufConvert`
...
- Storage in vega is now versioned. Old databases will not work with this
  update. (#707)
...
- `vega::crypto` types now have truncated `Display`/`Debug` representations. (#797)
...
- New kind of CLI commands has been added: `info` command that can be used for
  ...
  - `core-version` - prints Vega version as a plain string.
  - `list-services` - prints the list of the services the node is built with in
    the JSON format.

- `vega::crypto::x25519` module to convert keys from Ed25519 to X25519 format
...
- Dependencies versions have been updated:
  - `vega_sodiumoxide` to `0.0.19`.
  - `vega_rocksdb` to `0.7.4`.
...
- The service has been refactored and the following public structs has been
  moved to separate modules: `TimeSchema` to `vega_time::schema`,
  `TimeProvider` and `MockTimeProvider` to `vega_time::time_provider`,
  `ValidatorTime` to `vega_time::api`. (#604)
...
- `Field`, `CryptoHash`, `StorageValue` and `vegaJson` traits have been
  implemented for `chrono::Duration` structure. (#653)

- `before_commit` method has been added in `Service` trait. (#667) (#715)

- `Field`, `CryptoHash`, `StorageKey`, `StorageValue` and `vegaJson` traits
...
- `vega::explorer` module has been reworked to add new functionality.
...
- `majority_count` parameter has been added to the `StoredConfiguration`. See
  `vega-configuration` changes for more details. (#546)
...
- Added `v1/user_agent` endpoint with information about Vega, Rust and OS
  versions. (#548)
...
- Sandbox tests have been moved inside of the vega core. (#568)
...
- `vega::crypto::CryptoHash` trait is introduced, and `StorageValue::hash`
  ...
  - Add `use vega::crypto::CryptoHash` to use the `hash` method.
...
- Most types renamed to avoid stuttering (see [here][stuttering] for
  ...
  **Migration path:** Rename imported types from the crate, using aliases
  or qualified names if necessary: `use vega_configuration::Service as ConfigService`.
...
- `vegaJsonDeserialize` trait is implemented for `F32` and `F64`. (#461)
...
- `from_raw(raw: RawMessage)`  method is moved to the `Message` trait.
  To migrate, add `use vega::messages::Message`. (#427)
...
- The `Transaction` trait now inherits from `vegaJson`. (#402)
...
- Implemented `AsRef<RawMessage>` for any Vega messages that were
