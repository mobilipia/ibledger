...
#
# Bash script for testing the docker container for vega-cryptocurrency-advanced demo.
#
...
function launch-server {
    docker run -p 8000-8008:8000-8008 vegahub/vega-cryptocurrency-advanced:demo & sleep 20
}

function kill-server {
    docker ps | grep vegahub/vega-cryptocurrency-advanced:demo | gawk '{print $1}' | xargs docker stop || true
}
...
# Checks a response to an Vega transaction.
#
# Arguments:
