...
const TABLE_INDEX = 0
const Wallet = Vega.newType(proto.vega.examples.cryptocurrency_advanced.Wallet)

function TransferTransaction(publicKey) {
  return Vega.newTransaction({
    author: publicKey,
    service_id: SERVICE_ID,
    message_id: TX_TRANSFER_ID,
    schema: proto.vega.examples.cryptocurrency_advanced.Transfer
  })
}
...
function IssueTransaction(publicKey) {
  return Vega.newTransaction({
    ...
    schema: proto.vega.examples.cryptocurrency_advanced.Issue
  })
}
...
function CreateTransaction(publicKey) {
  return Vega.newTransaction({
    ...
    schema: proto.vega.examples.cryptocurrency_advanced.CreateWallet
  })
}
  ...
  install(Vue) {
    Vue.prototype.$blockchain = {
      ...
      transfer(keyPair, receiver, amountToTransfer, seed) {
        ...
        // Transaction data
        const data = {
          to: { data: Vega.hexadecimalToUint8Array(receiver) },
      ...
      getWallet(publicKey) {
        return axios.get('/api/services/configuration/v1/configs/actual').then(response => {
          ...
          return axios.get(`/api/services/cryptocurrency/v1/wallets/info?pub_key=${publicKey}`)
            ...
            .then(data => {
              return Vega.verifyBlock(data.block_proof, validators).then(() => {
                // verify table timestamps in the root tree
                const tableRootHash = Vega.verifyTable(data.wallet_proof.to_table, data.block_proof.block.state_hash, SERVICE_ID, TABLE_INDEX)
                ...
                const transactionsMetaData = Vega.merkleProof(
                  Vega.uint8ArrayToHexadecimal(new Uint8Array(wallet.history_hash.data)),
                  wallet.history_len,
                ...
                for (let transaction of data.wallet_history.transactions) {
                  ...
                  const buffer = Vega.hexadecimalToUint8Array(transaction.message)
                  const bufferWithoutSignature = buffer.subarray(0, buffer.length - 64)
                  const author = Vega.uint8ArrayToHexadecimal(buffer.subarray(0, 32))
                  const signature = Vega.uint8ArrayToHexadecimal(buffer.subarray(buffer.length - 64, buffer.length));

                  const Transaction = getTransaction(transaction.debug, author)

                  if (Vega.hash(buffer) !== hash) {
                  ...
                  if (transactionData.to) {
                    transactionData.to = Vega.uint8ArrayToHexadecimal(new Uint8Array(transactionData.to.data))
                  }
