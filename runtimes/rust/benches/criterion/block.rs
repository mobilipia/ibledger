...
use vega_rust_runtime::{DefaultInstance, RustRuntime};
...
fn gen_keypair_from_rng(rng: &mut StdRng) -> (PublicKey, SecretKey) {
    use vega::crypto::{gen_keypair_from_seed, Seed, SEED_LENGTH};
...
mod timestamping {
    ...
    use vega_rust_runtime::{DefaultInstance, Service};
...
mod cryptocurrency {
    ...
    use vega_derive::{
        vega_interface, BinaryValue, ObjectHash, ServiceDispatcher, ServiceFactory,
    };
...
mod foreign_interface_call {
    ...
    use vega_derive::{vega_interface, ServiceDispatcher, ServiceFactory};
    use vega_rust_runtime::{RustRuntime, Service, ServiceFactory as _};
    use rand::rngs::StdRng;
    use tempfile::TempDir;
    ...
    #[vega_interface(interface = "Configure", auto_ids)]
    pub trait Configure<Ctx> {
        type Output;
    }

    #[vega_interface(interface = "Events", auto_ids)]
    pub trait Events<Ctx> {
        type Output;
    }

    #[vega_interface(interface = "ERC30Tokens", auto_ids)]
