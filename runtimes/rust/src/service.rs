...
/// Describes an Vega service instance.
///
...
pub trait Service: ServiceDispatcher + Debug + 'static {
    ...
    /// Attaches the request handlers of the service API to the Vega API schema.
    ///
...
/// handler or the HTTP API handlers and broadcast them to the connected Vega nodes.
...
fn is_supervisor(instance_id: InstanceId) -> bool {
    instance_id == vega::runtime::SUPERVISOR_INSTANCE_ID
}
