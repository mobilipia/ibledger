...
use vega::{
    proto::schema::{INCLUDES as vega_INCLUDES, PROTO_SOURCES as vega_PROTO_SOURCES},
    runtime::{versioning::Version, ArtifactId, RuntimeIdentifier},
};
use vega_api::{self as api, ApiBuilder};
...
/// Artifact Protobuf specification for the Vega clients.
...
fn vega_proto_sources() -> Vec<ProtoSourceFile> {
    let proto = vega_PROTO_SOURCES
        .as_ref()
        .iter()
        .map(From::from)
        .collect::<Vec<_>>();
    let includes = vega_INCLUDES
        .as_ref()
        .iter()
        .map(From::from)
        .collect::<Vec<_>>();
...
fn filter_vega_proto_sources(
    files: Vec<ProtoSourceFile>,
    vega_sources: &[ProtoSourceFile],
) -> Vec<ProtoSourceFile> {
    files
        .into_iter()
        .filter(|file| !vega_sources.contains(file))
        .collect()
}
...
pub fn endpoints(runtime: &RustRuntime) -> impl IntoIterator<Item = (String, ApiBuilder)> {
    let artifact_proto_sources = runtime
        ...
        .collect::<HashMap<_, _>>();
    let vega_sources = vega_proto_sources();
    ...
    let filtered_sources = artifact_proto_sources
        ...
        .map(|(artifact_id, sources)| {
            ...
            proto.extend(filter_vega_proto_sources(
                sources.includes,
                &vega_sources,
            ));
            (artifact_id, proto)
    ...
    builder
        ...
        // otherwise it returns source files of Vega itself.
