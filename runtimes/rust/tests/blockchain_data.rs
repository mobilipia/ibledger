...
use vega_derive::{FromAccess, ServiceDispatcher, ServiceFactory};
use futures::sync::mpsc;

use std::collections::BTreeMap;

use vega_rust_runtime::{DefaultInstance, RustRuntimeBuilder, Service, ServiceFactory};
...
#[service_factory(artifact_name = "vega.Token", artifact_version = "1.4.0")]
struct TokenService;
...
#[derive(Debug, ServiceDispatcher, ServiceFactory)]
#[service_factory(artifact_name = "vega.Token", artifact_version = "1.0.0")]
struct OldService;
...
#[derive(Debug, ServiceDispatcher, ServiceFactory)]
#[service_factory(artifact_name = "vega.OtherService", artifact_version = "1.3.5")]
struct OtherService;
...
fn access_to_service_schema() {
    ...
    assert_matches!(
        err,
        ArtifactReqError::UnexpectedName { ref actual, .. } if actual == "vega.OtherService"
    );
