...
from vega_client import ModuleManager, vegaClient, MessageGenerator
from vega_client.crypto import Hash


class vegaCryptoAdvancedClient:
    """Class provides an interface to simplify interaction with cryptocurrency-advanced service."""

    def __init__(self, client: vegaClient):
        self.client = client
        cryptocurrency_service_name = "vega-cryptocurrency-advanced"
        cryptocurrency_service_version = "0.1.0"
        self.loader = client.protobuf_loader()
        self.loader.initialize()
        self.loader.load_main_proto_files()
        self.loader.load_service_proto_files(
            runtime_id=0,
            artifact_name="vega-supervisor",
