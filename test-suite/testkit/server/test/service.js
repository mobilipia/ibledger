...
const vega = require('vega-client')
...
function haveTxBody (type, data, secretKey) {
  // Clone type.
  const typeCopy = vega.newTransaction(type)
  // Sign transaction.
  ...
  const txBody = vega.uint8ArrayToHexadecimal(new Uint8Array(buffer))
  // Get transaction hash.
  const txHash = vega.hash(buffer)
...
exports.service = {
  createWalletTransaction (author) {
    const tx = vega.newTransaction({
      service_id: SERVICE_ID,
      message_id: TX_CREATE_WALLET_ID,
      author,
      schema: proto.vega.examples.cryptocurrency.TxCreateWallet
    })
  ...
  createTransferTransaction (author) {
    const tx = vega.newTransaction({
      ...
      author,
      schema: proto.vega.examples.cryptocurrency.TxTransfer
    })
