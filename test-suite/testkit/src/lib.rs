...
//! Testkit for Vega blockchain framework, allowing to test service APIs synchronously
...
//! use serde_derive::*;
//! use vega_derive::*;
//! use vega_merkledb::{ObjectHash, Snapshot};
//! use vega_testkit::{ApiKind, TestKitBuilder};
//! use vega_rust_runtime::{ServiceFactory, ExecutionContext, Service};
//!
...
pub use vega_explorer as explorer;
...
use vega_rust_runtime::{RustRuntimeBuilder, ServiceFactory};
use futures::{sync::mpsc, Future, Stream};
use tokio_core::reactor::Core;

#[cfg(feature = "vega-node")]
use vega_node::{ExternalMessage, NodePlugin, PluginApiContext, SharedNodeState};
...
impl TestKit {
    ...
    ///
    /// This method is only available if the crate is compiled with the `vega-node` feature.
    #[cfg(feature = "vega-node")]
    ...
    /// # use vega_derive::{vega_interface, interface_method, ServiceFactory, ServiceDispatcher, BinaryValue};
    /// # use vega_testkit::{TestKit, TestKitBuilder};
    /// # use vega_merkledb::Snapshot;
    /// # use vega::{crypto::{PublicKey, Hash, SecretKey}, runtime::ExecutionError};
    /// # use vega_rust_runtime::{ExecutionContext, Service, ServiceFactory};
    /// #
    ...
    /// let keys = vega::crypto::gen_keypair();
    ...
    ///
    /// ```
    /// # extern crate vega_testkit;
    /// # extern crate vega;
    /// # use vega::helpers::Height;
    /// # use vega_testkit::TestKitBuilder;
    /// # fn main() {
...
/// Persistent state of an Vega node allowing to emulate node restart.
///
...
///
/// ```
/// # use vega_derive::{vega_interface, ServiceFactory, ServiceDispatcher};
/// # use vega::{
/// #     crypto::{PublicKey, Hash},
/// #     helpers::Height,
/// #     runtime::BlockchainData,
/// # };
/// # use vega_rust_runtime::{AfterCommitContext, RustRuntime, Service};
/// # use vega_merkledb::{Fork, Snapshot};
/// # use vega_testkit::{StoppedTestKit, TestKit};
...
impl StoppedTestKit {
    ...
    /// (which is also what may happen with real Vega apps).
