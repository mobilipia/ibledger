...
use vega_rust_runtime::{ProtoSourceFile, ProtoSourcesQuery};
use vega_testkit::{ApiKind, TestKit, TestKitApi, TestKitBuilder};
...
pub fn assert_vega_core_protos(api: &TestKitApi) {
...
fn core_protos_without_services() {
    ...
    assert_vega_core_protos(&testkit.api());
}
...
fn service_protos_with_incorrect_service() {
    use vega::runtime::{ArtifactId, RuntimeIdentifier};
