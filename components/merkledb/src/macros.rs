        ...
        $(
            impl ObjectHash for $type {
                fn object_hash(&self) -> Hash {
                    vega_crypto::hash(&self.to_bytes())
                }
            }
        )*
    };
...
macro_rules! impl_binary_key_for_binary_value {
    ($type:ty) => {
        impl vega_merkledb::BinaryKey for $type {
            fn size(&self) -> usize {
                vega_merkledb::BinaryValue::to_bytes(self).len()
            }

            fn write(&self, buffer: &mut [u8]) -> usize {
                let mut bytes = vega_merkledb::BinaryValue::to_bytes(self);
            ...
            fn read(buffer: &[u8]) -> Self::Owned {
                ...
                <Self as vega_merkledb::BinaryValue>::from_bytes(buffer.into()).unwrap()
            }
        }
    };
}
