...
/// as untrusted inputs. Implementations may rely on the invariants provided by Vega nodes
    ...
    KeyMode: ToProofPath<K>,
{
    ...
    ///
    /// ```
    /// # use vega_merkledb::{access::AccessExt, Database, TemporaryDB, ProofMapIndex, ObjectHash};
    /// # use vega_crypto::hash;
    /// let fork = { let db = TemporaryDB::new(); db.fork() };
    /// let mut map = fork.get_proof_map("index");
