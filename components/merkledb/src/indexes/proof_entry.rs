    ...
    V: BinaryValue,
{
    ...
    ///
    /// ```
    /// use vega_merkledb::{access::AccessExt, TemporaryDB, Database, Entry};
    ///
...
///
/// ```
/// # use vega_merkledb::{access::AccessExt, TemporaryDB, Database, Entry, ObjectHash};
/// # use vega_crypto::{self, Hash};
/// let db = TemporaryDB::new();
/// let fork = db.fork();
/// let mut index = fork.get_proof_entry("name");
...
/// let value = 10;
/// index.set(value);
/// assert_eq!(vega_crypto::hash(&[value]), index.object_hash());
/// ```
...
mod tests {
    ...
    fn entry_with_custom_hashing() {
        ...
        impl ObjectHash for CustomHash {
            fn object_hash(&self) -> Hash {
                Hash::new([self.0; vega_crypto::HASH_SIZE])
            }
        }
        ...
        assert_eq!(
            entry.object_hash(),
            Hash::new([11; vega_crypto::HASH_SIZE])
        );
    }
}
