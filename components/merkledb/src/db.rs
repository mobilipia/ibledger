...
///
/// ```
/// # use vega_merkledb::{access::AccessExt, Database, TemporaryDB};
...
///
/// ```rust,should_panic
/// # use vega_merkledb::{access::AccessExt, TemporaryDB, ListIndex, Database};
/// let db = TemporaryDB::new();
/// let fork = db.fork();
...
pub trait DatabaseExt: Database {
    ...
    ///
    /// ```
    /// # use vega_merkledb::{access::{Access, AccessExt}, Database, DatabaseExt, TemporaryDB};
    /// let db = TemporaryDB::new();
    /// let fork = db.fork();
...
/// trait, `Iterator` in Vega is low-level and, therefore, operates with bytes.
pub trait Iterator {
...
///
/// ```
/// # use vega_merkledb::{access::AccessExt, Database, ReadonlyFork, TemporaryDB};
/// let db = TemporaryDB::new();
/// let fork = db.fork();
/// fork.get_list("list").push(1_u32);
