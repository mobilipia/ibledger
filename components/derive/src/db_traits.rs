...
impl ObjectHashStruct {
    pub fn implement_object_hash(&self) -> impl ToTokens {
        ...
        quote! {
            impl vega_merkledb::ObjectHash for #name {
                fn object_hash(&self) -> vega_merkledb::_reexports::Hash {
                    let bytes = vega_merkledb::BinaryValue::to_bytes(self);
                    vega_merkledb::_reexports::hash(&bytes)
                }
            }
        }
    }
...
impl BinaryValueStruct {
    fn implement_binary_value_from_pb(&self) -> proc_macro2::TokenStream {
        ...
        quote! {
            impl vega_merkledb::BinaryValue for #name {
                fn to_bytes(&self) -> Vec<u8> {
                    use protobuf::Message as _;
                    // This trait assumes that we work with trusted data so we can unwrap here.
                    vega_proto::ProtobufConvert::to_pb(self).write_to_bytes().expect(
                ...
                ) -> std::result::Result<Self, vega_merkledb::_reexports::Error> {
                    use protobuf::Message as _;

                    let mut block = <Self as vega_proto::ProtobufConvert>::ProtoStruct::new();
                    block.merge_from_bytes(value.as_ref())?;
                    vega_proto::ProtobufConvert::from_pb(block)
                }
            }
        }
    }
...
impl AccessField {
    ...
    fn constructor(&self, field_index: usize) -> impl ToTokens {
        let from_access = quote!(vega_merkledb::access::FromAccess);
...
impl FromAccess {
    fn access_fn(&self) -> impl ToTokens {
        ...
        quote! {
            fn from_access(
                access: #access_ident,
                addr: vega_merkledb::IndexAddress,
            ) -> Result<Self, vega_merkledb::access::AccessError> {
                #fn_impl
            }
        }
    }
...
impl ToTokens for FromAccess {
    fn to_tokens(&self, tokens: &mut proc_macro2::TokenStream) {
        let name = &self.ident;
        let tr = quote!(vega_merkledb::access::FromAccess);
        let access_ident = &self.access_ident;
