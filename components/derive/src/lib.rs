...
//! This crate provides macros for deriving some useful methods and traits for the vega services.
...
///
/// - Protobuf serialization (used by default) via `vega-proto` crate and its `ProtobufConvert`
///   trait.
...
/// ```
///
/// Prefix of the `vega` crate has two main values - `crate` or `vega`. The default value
/// is `vega`.
...
/// Derives an Vega service interface for the specified trait.
///
/// See the documentation of the Vega crate for more information.
///
/// # Attributes
///
/// ## `crate`
...
///
/// ```text
/// #[vega_interface(removed_method_ids(0, 2, 5))]
/// ```
///
...
/// ```
///
/// All the method in the trait with `vega_interface` attribute should have `interface_method`
/// attribute with unsigned integer value. All the method IDs should be unique.
#[proc_macro_attribute]
pub fn vega_interface(attr: TokenStream, item: TokenStream) -> TokenStream {
    vega_interface::impl_vega_interface(attr, item)
}
...
pub fn interface_method(_attr: TokenStream, item: TokenStream) -> TokenStream {
    ...
    // provides additional metadata for `vega_interface` attribute.
    //
...
impl Default for MainCratePath {
    fn default() -> Self {
        Self(syn::parse_str("vega").unwrap())
    }
}
...
impl Default for RustRuntimeCratePath {
    fn default() -> Self {
        Self(syn::parse_str("vega_rust_runtime").unwrap())
    }
}
