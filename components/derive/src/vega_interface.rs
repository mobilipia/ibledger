...
impl Default for VegaInterfaceAttrs {
    fn default() -> Self {
        Self {
...
impl TryFrom<&[Attribute]> for VegaInterfaceAttrs {
    type Error = darling::Error;

    fn try_from(args: &[Attribute]) -> Result<Self, Self::Error> {
        find_meta_attrs("vega", args)
...
#[derive(Debug)]
struct VegaInterface {
    item_trait: ItemTrait,
    attrs: VegaInterfaceAttrs,
...
impl VegaInterface {
    fn new(item_trait: ItemTrait, args: Vec<NestedMeta>) -> Result<Self, darling::Error> {
        ...
        // Extract attributes.
        let attrs = VegaInterfaceAttrs::from_list(&args)?;
        ...
        for trait_item in &item_trait.items {
            match trait_item {
                ...
                _ => {
                    let msg = "Unsupported item in an Vega interface";
    ...
    fn impl_interface(&self) -> impl ToTokens {
        ...
        let impl_match_arm_for_method = |descriptor: &ServiceMethodDescriptor| {
            ...
            quote! {
                #id => {
                    let arg: #arg_type = vega::merkledb::BinaryValue::from_bytes(payload.into())
                        .map_err(vega::runtime::CommonError::malformed_arguments)?;
        ...
        let impl_match_arm_for_removed_method = |id: &u32| {
            quote! {
                #id => {
                    return Err(vega::runtime::CommonError::MethodRemoved.into());
                }
            }
        };
        ...
        let res = quote!(std::result::Result<(), vega::runtime::ExecutionError>);
        quote! {
            impl<'a> #cr::Interface<'a> for dyn #trait_name<#ctx, Output = #res> {
                ...
                fn dispatch(
                    ...
                    method: vega::runtime::MethodId,
                    payload: &[u8],
                ) -> #res {
                    match method {
                        #( #match_arms )*
                        #( #removed_match_arms )*
                        _ => Err(vega::runtime::CommonError::NoSuchMethod.into()),
                    }
                }
            }
        }
            ...
            let method = quote! {
                fn #name(&self, context: Ctx, arg: #arg_type) -> Self::Output {
                    #cr::GenericCall::generic_call(
                        ...
                        context,
                        #descriptor,
                        vega::merkledb::BinaryValue::into_bytes(arg),
                    )
                }
            };
...
impl ToTokens for VegaInterface {
...
pub fn impl_vega_interface(attr: TokenStream, item: TokenStream) -> TokenStream {
    let item_trait = parse_macro_input!(item as ItemTrait);
    let attrs = parse_macro_input!(attr as AttributeArgs);

    let vega_interface = match VegaInterface::new(item_trait, attrs) {
        Ok(vega_interface) => vega_interface,
        Err(e) => return e.write_errors().into(),
    };
    let tokens = quote!(#vega_interface);
    tokens.into()
}
