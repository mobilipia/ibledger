...
use vega_rust_runtime::{DefaultInstance, RustRuntimeBuilder, ServiceFactory};
...
use crate::{
    ...
    messages::{
        BlockRequest, BlockResponse, Connect, vegaMessage, Message, PeersRequest,
                ...
                match internal {
                    ...
                    InternalRequest::VerifyMessage(raw) => {
                        let msg = SignedMessage::from_bytes(raw.into())
                            .and_then(SignedMessage::into_verified::<vegaMessage>)
                            .map(Message::from)
                            .unwrap();
...
impl Default for SandboxBuilder {
    fn default() -> Self {
        use vega::blockchain::ConsensusConfigBuilder;
