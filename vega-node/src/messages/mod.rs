...
/// Representation of the Vega message which is divided into categories.
#[derive(Debug, Clone, PartialEq)]
pub enum Message {
    /// Service messages.
    Service(Service),
...
impl Message {
    ...
    pub fn from_signed(signed: SignedMessage) -> Result<Self, failure::Error> {
        signed.into_verified::<vegaMessage>().map(From::from)
    }
...
macro_rules! impl_message_from_verified {
    ( $($concrete:ident: $category:ident),* ) => {
        ...
        impl From<Verified<vegaMessage>> for Message {
            fn from(msg: Verified<vegaMessage>) -> Self {
                match msg.payload() {
                    $(
                        vegaMessage::$concrete(_) => {
                            let inner = msg.downcast_map(|payload| match payload {
                                vegaMessage::$concrete(payload) => payload,
                                _ => unreachable!(),
                            });
                            Self::from(inner)
                        }
...
mod tests {
    ...
    fn test_verified_from_signed_correct_signature() {
        ...
        let protocol_message = vegaMessage::from(msg.clone());
        let signed = SignedMessage::new(protocol_message.clone(), keypair.0, &keypair.1);

        let verified_protocol = signed.clone().into_verified::<vegaMessage>().unwrap();
    ...
    fn test_verified_from_signed_incorrect_signature() {
        ...
        let err = signed.clone().into_verified::<vegaMessage>().unwrap_err();
