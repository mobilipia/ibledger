...
//! Vega node that handles consensus algorithm, interaction with other nodes and external clients.
//!
//! # Overview
//!
//! This crate contains the following APIs:
//!
//! - [`Node`] encapsulates a full-fledged Vega node
...
/// # Stability
///
/// This type and its methods are considered an implementation detail of the Vega node and are
...
/// P2P network configuration of an Vega node.
...
pub fn generate_testnet_config(count: u16, start_port: u16) -> Vec<NodeConfig> {
    use vega::{blockchain::ValidatorKeys, crypto::gen_keypair};
...
#[cfg(test)]
mod tests {
    use vega::{blockchain::config::GenesisConfigBuilder, merkledb::TemporaryDB};
