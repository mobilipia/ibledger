# Library for building CLI for Vega node
...
`vega-cli` crate provides an extensible command line interface for Vega
nodes.
...
However, new commands can be added to extend the `vega-cli` functionality.

Consult [the crate docs](https://docs.rs/vega-cli) for more details.
...
2. Generate public and secret (private) parts of the node configuration using
  `generate-config` command. At this step, Vega will generate master key, from
...
3. Generate final node configuration using `finalize` command. Vega combines
...
`vega-cli` also supports additional CLI commands for performing maintenance
...
Running an Vega node with default settings:
...
Include `vega-cli` as a dependency in your `Cargo.toml`:
...
`vega-cli` is licensed under the Apache License (Version 2.0).
