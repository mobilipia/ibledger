...
//! Standard Vega CLI command used to generate public and secret config files
...
use crate::{
    command::{vegaCommand, StandardResult},
...
/// Default port number used by Vega for communication between nodes.
pub const DEFAULT_vega_LISTEN_PORT: u16 = 6333;
...
pub struct GenerateConfig {
    ...
    ///
    /// If no port is provided, the default Vega port 6333 is used.
    #[structopt(
        long,
        short = "a",
    ...
    /// If `ENV_VAR_NAME` is not specified `$vega_MASTER_PASS` is used
    /// by default.
    #[structopt(long)]
...
impl vegaCommand for GenerateConfig {
