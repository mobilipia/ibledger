...
//! Helper crate for secure and convenient configuration of the Vega nodes.
//!
//! `vega-cli` supports multi-stage configuration process made with safety in mind. It involves
...
//!   command. At this step, Vega will generate master key from which consensus and service
...
//! 3. Generate final node configuration using `finalize` command. Vega combines secret part of
...
//! ## Additional Commands
//!
//! `vega-cli` also supports additional CLI commands for performing maintenance actions by node
...
//!
//! `vega-cli` allows to extend the list of the parameters for any command and even add new CLI
...
//! accessing node configuration files created and filled by the standard Vega commands.
//!
...
use vega_node::NodeBuilder as CoreNodeBuilder;
use vega_rust_runtime::{DefaultInstance, RustRuntimeBuilder, ServiceFactory};
use vega_supervisor::{Supervisor, SupervisorConfig};
use vega_system_api::SystemApiPlugin;

use crate::command::{run::NodeRunConfig, Command, vegaCommand, StandardResult};
