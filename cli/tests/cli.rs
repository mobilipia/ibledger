...
// This is a regression test for vega configuration.
...
use vega_cli::{
    command::{
        ...
        run::Run, Command, vegaCommand, StandardResult,
    },
...
use vega_supervisor::mode::Mode as SupervisorMode;
...
impl ConfigSpec {
    ...
    fn command(&self, name: &str) -> ArgsBuilder {
        ArgsBuilder {
            args: vec!["vega-config-test".into(), name.into()],
        }
    }
...
fn test_full_workflow() {
    ...
    env::set_var("vega_MASTER_PASS", "12345678");
    for i in 0..env.validators_count {
        let node_config = env.output_node_config(i);
        env.command("finalize")
            .with_arg(env.output_private_config(i))
