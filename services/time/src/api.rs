...
//!
//! Returns the current stored time available in `vega-time` service.
//! `None` will be returned if there is no enough data to provide a trusted
//! time yet.
//!
//! ```
//! # use chrono::{DateTime, Utc};
//! # use vega::{helpers::Height, runtime::InstanceId};
...
//! # use vega_rust_runtime::ServiceFactory;
//! # use vega_testkit::{ApiKind, TestKit, TestKitBuilder};
//! # use vega_time::{TimeServiceFactory, ValidatorTime};
//! # const TIME_SERVICE_ID: InstanceId = 100;
//! const TIME_SERVICE_NAME: &'static str = "time-oracle";
//!
//! # fn main() -> Result<(), failure::Error> {
...
/// Implement the public API for Vega time.
#[derive(Debug, Clone)]
...
/// Implement the private API for Vega time.
#[derive(Debug, Clone)]
