...
use vega_merkledb::{impl_serde_hex_for_binary_value, BinaryValue, ObjectHash};
    ...
    T: TryFrom<SignedMessage> + IntoMessage,
{
    ...
    pub fn from_value(inner: T, public_key: PublicKey, secret_key: &SecretKey) -> Self {
        ...
        let inner: T = if let Ok(inner) = container.try_into() {
            ...
            unreachable!("We can safely convert `vegaMessage` back to the inner type.")
        };
        Self { raw, inner }
    }
}
...
mod tests {
    use chrono::Utc;
    use vega_crypto::{self as crypto, Signature};
