...
//! Versioning tools for Vega artifacts.
//!
//! # Versioning Problem
//!
//! The problem solved by versioning is as follows. Vega services have clients, both internal
...
//! # Artifact versioning
//!
//! For any reasonable solution to the problem above to work, Vega artifacts **must** be
...
//!
//! - Versioning concerns *all* public interfaces of the service. As of Vega 1.0, these interfaces
...
//!
//! At the same time, Vega core does not provide a tool for marking transaction as deprecated.
...
//!
//! To defend against these scenarios, Vega provides following defences.
//!
...
///
/// ```
/// # use vega::runtime::{versioning::ArtifactReq, ArtifactId, RuntimeIdentifier};
...
/// # Examples
///
/// This trait is usually implemented via the derive macro from the `vega_derive` crate:
///
/// ```
/// use vega_derive::*;
/// # use vega_merkledb::{access::Access, Fork, ProofMapIndex};
/// # use vega::runtime::versioning::RequireArtifact;
///
/// #[derive(Debug, FromAccess, RequireArtifact)]
...
/// ```
/// # use vega_derive::*;
/// # use vega_merkledb::{access::Access, Fork, ProofEntry, ProofMapIndex};
/// # use vega::runtime::versioning::RequireArtifact;
/// #[derive(Debug, FromAccess, RequireArtifact)]
...
fn artifact_req_parsing() {
    ...
    let req: ArtifactReq = "vega.Token@^1.0.5".parse().unwrap();
    assert_eq!(req.name, "vega.Token");
    assert_eq!(req.version, "^1.0.5".parse().unwrap());
    assert_eq!(req.to_string(), "vega.Token@^1.0.5");
}
