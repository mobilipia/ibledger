...
fn create_path_to_protobuf_schema_env() {
    ...
    // We "link" vega with vega_protobuf library
    // and dependents in their `build.rs` will have access to `$DEP_vega_PROTOBUF_PROTOS`.

    let current_dir = env::current_dir().expect("Failed to get current dir.");
    let protos = current_dir.join("src/proto/schema/vega");
    println!("cargo:protos={}", protos.to_str().unwrap());

    // Reexport common, MerkleDB and crypto protobuf files.
    let common_protos = env::var("DEP_vega_PROTOBUF_COMMON_PROTOS")
        .expect("Cannot obtain `common` protobuf files");
    println!("cargo:common_protos={}", common_protos);
    let crypto_protos = env::var("DEP_vega_PROTOBUF_CRYPTO_PROTOS")
        .expect("Cannot obtain `crypto` protobuf files");
    println!("cargo:crypto_protos={}", crypto_protos);
    let merkledb_protos = env::var("DEP_vega_PROTOBUF_MERKLEDB_PROTOS")
        .expect("Cannot obtain `merkledb` protobuf files");
    println!("cargo:merkledb_protos={}", merkledb_protos);
}

fn write_user_agent_file() {
    let package_name = option_env!("CARGO_PKG_NAME").unwrap_or("vega");
...
fn main() {
    ...
    ProtobufGenerator::with_mod_name("vega_proto_mod.rs")
        .with_input_dir("src/proto/schema/vega")
        .with_crypto()
        .with_common()
        .with_merkledb()
        .generate();
